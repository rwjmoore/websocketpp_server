...
// Function to convert a string of hexadecimal values (with spaces) to a vector of bytes
std::vector<byte> hexStringToBytes(const std::string& hexString) {
    std::vector<byte> bytes;
    std::istringstream iss(hexString);
    std::string token;
    while (iss >> std::skipws >> token) { // Skip whitespace
        try {
            byte b = static_cast<byte>(std::stoul(token, nullptr, 16));
            bytes.push_back(b);
        } catch (const std::invalid_argument& e) {
            throw std::invalid_argument("Invalid hexadecimal string: " + hexString);
        }
    }
    return bytes;
}
